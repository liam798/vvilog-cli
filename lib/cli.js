...
const { installAllSkillsToTargets, installSkillToTargets, listAvailableSkills, listInstalledVviLogSkills, removeSkillFromTargets } = require("./skills");
...
async function main(argv) {
  ...
  if (command === "doctor") {
    ...
    emit(jsonOutput, doctor({ sourceDir: options.sourceDir }));
    return;
  }
  if (command === "init") {
    const options = parseOptions([subcommand, ...rest].filter(Boolean), {
...
async function handleSkills(subcommand, argv, jsonOutput) {
  ...
  if (subcommand === "remove") {
    ...
    emit(jsonOutput, removeSkillFromTargets({ skill: name, target: options.target, dryRun: options.dryRun }));
    return;
  }
